...
def main():
    ...
    bestand = "seqschamp.txt"    #tab delimited text file van excel bestand 100 sequenties groep 1
    b = open(bestand, "r")
    a = ""
    count = 1
    seqs = []
...
def boolean(b, head):
     ...
     cursor = conn.cursor()
     query = ("UPDATE `pg3`.`main_seq` SET `match_found`='"+str(a)+"' WHERE `HWI_code`='"+str(head)+"';")   
     print (query)
...
def source_seq(bool, head, seq): 
     conn = mysql.connector.connect(host="ithurtswheip.nl",user = "richard", password = "richard", db = "blast", port = 3307)
     cursor = conn.cursor()
     query = ("INSERT INTO `pg3`.`main_seq`(`HWI_code`, `match_found`, `seq`) VALUES ('"+str(head)+"', "+str(bool)+",'"+str(seq)+"');")
     cursor.execute(query)   
     conn.commit()
     print (query)
     cursor.close()
...
# 		        print ("\n***********************************************\n")
                    if c == 1:
                       # print ("INSERT INTO `blast`.`BLAST_RESULT`(`amount-hits`, `result1-accession-code`, `result1-alignment-score`, `result1-query-coverage-percentage`, `result1-E-value`, `result1-ident-percentage`, `SOURCE_SEQ_seq-id`) VALUES ("+str(numhits)+", '"+str(hithead)+"', "+str(score)+", "+str(qcov)+", "+str(evalue)+", "+str(identity)+",'"+str(head)+"');")
                        conn = mysql.connector.connect(host="ithurtswhenip.nl",user = "richard", password = "richard", db = "blast", port = 3307)
                        cursor = conn.cursor()
                        query1 = ("INSERT INTO `pg3`.`results_blast`(`result_found`, `result1_accession_code_result`, `result1_score`, \
`result1_gaps`, `result1_E_value`, `result1_identity`, `main_seq_HWI_code`) VALUES ("+str(numhits)+", '"+str(hithead)+"', "+str(score)+",\
"+str(gaps)+", "+str(evalue)+", "+str(identity)+",'"+str(head)+"');")
                       ...
                       #  print (rows)
                        ...
                        conn = mysql.connector.connect(host="ithurtswhenip.nl",user = "richard", password = "richard", db = "blast", port = 3307)
                        cursor = conn.cursor()
                        query2 = ("UPDATE `pg3`.`results_blast` SET `result"+str(c)+"_accession_code_result`='"+str(hithead)+"', \
`result"+str(c)+"_score`='"+str(score)+"', `result"+str(c)+"-gaps`='"+str(gaps)+"', \
`result"+str(c)+"_E_value`='"+str(evalue)+"', `result"+str(c)+"_identity`='"+str(identity)+"' WHERE \
`main_seq_HWI_code`='"+str(head)+"';")
